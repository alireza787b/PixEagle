...
def test_browser_user_cli_updates_password_role_enabled_and_remove(tmp_path):
    ...
    assert "WARNING: no enabled browser-session users remain" in disable.stdout
    assert load_user_records(user_file)[0].enabled is False
    ...
    assert "WARNING: no enabled browser-session users remain" in remove.stdout
    assert load_user_records(user_file) == ()

...
test('renders SIH validation status, commands, and latest manifest evidence', async () => {
  ...
  expect(await screen.findByText('PX4 SIH container')).toBeInTheDocument();
...
test('renders no-manifest guidance with lifecycle actions derived from readiness', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Start PX4 container' })).toBeEnabled();
});
...
test('starts only after the explicit no-real-aircraft confirmation', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: 'Start PX4 container' }));
...
test('stops an owned simulator without the start-only hardware acknowledgement', async () => {
  ...
  fireEvent.click(await screen.findByRole('button', { name: 'Stop PX4 container' }));
...
test('keeps lifecycle mutations disabled without system administrator scope', async () => {
  ...
  expect(await screen.findByRole('button', { name: 'Start PX4 container' })).toBeDisabled();
  expect(screen.getByRole('button', { name: 'Stop PX4 container' })).toBeDisabled();
});

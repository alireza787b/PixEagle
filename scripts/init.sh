...
show_summary() {
    ...
    echo -e "      - Raw developer default: ${BOLD}http://127.0.0.1:3040${NC} (local-only; no account is created)"
    echo -e "      - Guided authenticated local lab: ${BOLD}cd $project_cmd_dir && make quick-browser-demo LAN_HOST=127.0.0.1${NC}"
    echo -e "      - Guided authenticated network lab: ${BOLD}cd $project_cmd_dir && make quick-browser-demo LAN_HOST=<device-ip>${NC}"
    echo -e "        ${DIM}Both ask for credentials; Enter keeps admin/admin. Generated mode creates a one-time password handoff.${NC}"
    echo ""

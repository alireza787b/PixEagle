...
| --- | --- | --- | --- |
| Phase 5 beta14 guided installer and GStreamer build acceptance recovery | in_progress | PXE-0115, PXE-0116 | Beta.13 maintainer evidence isolated discarded yes/no responses and a post-compile Python-bytecode source-digest false positive. Beta.14 code/tests/docs now repair both bootstrap readers, make Core plus only the shell shortcut the guided Enter path, reject ambiguous optional choices, fail closed on terminal loss, keep service/auto-start explicit, and suppress Python bytecode without weakening strict OpenCV source integrity or rollback. Local focused/setup/update/docs/Phase-0/schema/dashboard gates pass. Next: bounded independent review, exact clean-checkout handoff, publication, then maintainer rerun on disposable Ubuntu. A reviewed model downloader is deferred; the pinned manual lab example remains. |

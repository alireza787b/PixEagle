...
| --- | --- | --- | --- |
| Phase 5 beta15 service/runtime, CI hygiene, and dependency-policy recovery | done; final CI/publication gate pending | PXE-0117, PXE-0118, PXE-0119 | `checkpoints/2026-07-21-phase-5-beta15-service-readiness-closure.md` and the service-runtime checkpoint; exact `ea725c76` closes the real Ubuntu update/systemd/tmux gate after fixing tracked-source chmod mutation, no-PX4 MAVSDK readiness, update-owned service actions, bounded restart recovery, and status clarity. Maintained GitHub Actions are now immutable-pinned with monthly Dependabot tracking, and the stale gimbal CI example is corrected. The Full AI/GStreamer host is clean and active with `NRestarts=0`, a healthy four-pane contract, owned loopback listeners, successful dashboard/About/MJPEG probes, and zero error-like active JSONL entries. Publish beta.15 only after the post-pin final CI. Fresh beginner Ubuntu and exact-tag Raspberry Pi Core-first acceptance remain next; no PX4, QGC, remote WebRTC, field, or aircraft claim. |

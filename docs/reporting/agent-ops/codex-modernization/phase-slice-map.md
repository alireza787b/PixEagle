...
| --- | --- | --- | --- |
| Phase 5 beta15 service/runtime and dependency-policy recovery | in_progress | PXE-0117 | `checkpoints/2026-07-21-phase-5-beta15-service-runtime-onboarding-recovery.md`; private launcher handoff keeps canonical ownership markers off systemd and outer lock orchestration processes, tmux session creation publishes them atomically, the service checks healthy panes before readiness, `/proc` ownership reads tolerate process exit, PyTorch setup uses explicit conflict-warning flags without independent `setuptools` churn, and the OpenCV policy uses the correct distribution-version namespace. Service/manual/auth onboarding is explicit and unattended service selection fails closed. Exact candidate `f991a4c1` passed focused, Phase 0, schema, dashboard, and dashboard-inclusive clean-handoff gates (`27/27`). Publication remains blocked only on real disposable Ubuntu systemd/tmux repair evidence; current VPS authentication is unavailable. |

...
| 4 | Controlled VPS/browser readiness walkthrough | PXE-0068, PXE-0074 | Next recommended slice: run a clean temp-directory walkthrough on the VPS, choose SSH/local-only tunnel, private-overlay, or lab browser access for the first controlled review, generate credentials without leaking secrets only when the selected profile requires them, start services with verified binds, run browser smoke checks, and report sanitized evidence before handing the user a URL. |

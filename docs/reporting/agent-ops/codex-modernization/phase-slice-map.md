...
| 4 | User browser confirmation and demo credential cleanup | PXE-0068, PXE-0074 | Next recommended slice after the user test: record whether the user reached `http://100.82.207.49:3040`, then stop the tmux session and rotate/delete the demo handoff credential if the quick demo is finished. If public access is needed, move to the production HTTPS/WSS evidence path instead of broadening HTTP exposure. |

...
| Phase 4 runtime logging foundation | done/partial | PXE-0079 | `checkpoints/2026-07-04-phase-4-runtime-logging-foundation.md`; PixEagle now creates per-run runtime manifests and `components/backend.jsonl`, redacts common credentials, applies bounded retention, exports a shared launcher run ID, exposes read-only typed `/api/v1/logs/status`, `/api/v1/logs/sessions`, and `/api/v1/logs/sessions/{run_id}`, and adds a dashboard Logs page for operator/debug visibility. Remaining PXE-0079 work is dashboard/sidecar stdout capture, frontend error ingestion, live stream/export/evidence bundle, and clean setup evidence. Runtime logs are process-local evidence only, not PX4/SITL/HIL/field proof. |
...
| 4 | Setup/bootstrap UX consolidation | PXE-0078 | Done: Python dependencies are role-based (`requirements-core.txt`, `requirements-ai.txt`, `requirements-dev.txt`), stale optional dlib guidance is fixed, setup docs include a beginner/developer choice matrix, and quick-demo output now previews side effects, skipped sidecars, WebRTC/WebSocket expectations, and cleanup. `DRY_RUN=1` is no-touch and custom credential paths no longer chmod existing parent directories such as `/tmp`. Remaining target deployment and final clean-walkthrough evidence stay under PXE-0068/PXE-0074. |
| 4 | Unified runtime logging and evidence | PXE-0079 | Foundation done: runtime sessions/manifests, backend JSONL, retention/redaction, launcher run ID, typed read-only `/api/v1/logs/*`, and dashboard Logs page. Remaining work: dashboard/sidecar stdout capture, frontend error reports, live stream/export/evidence bundle, and final setup walkthrough evidence. Keep security audit separate and do not use runtime logs as flight proof without PX4/SITL/HIL artifacts. |

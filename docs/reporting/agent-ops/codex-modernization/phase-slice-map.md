...
same helper, preserving MJPEG session revocation and cleanup behavior. Remaining
PXE-0008 legacy route-body cleanup includes long-lived video WebSocket/WebRTC
transport route bodies and a separate circuit-breaker mutation cleanup before
typed `/api/v1` replacements or tracked compatibility retirement.

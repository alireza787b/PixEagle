...
compatibility retirement for the remaining legacy tracker diagnostic routes,
...
- PXE-0044: typed tracker runtime status now has a shared backend evaluator,
  ...
  `/api/tracker/current` at that checkpoint and still exist on
  `/api/tracker/current-status`, dashboard selector and status-hook adoption,
  reverse-proxy-safe tracker hooks, and target-loss fail-closed behavior for
  active+stale or active+not-usable input. `/api/tracker/current` was later
  retired on 2026-07-02. Done in
  `checkpoints/2026-06-05-phase-4-typed-tracker-runtime-status.md`.

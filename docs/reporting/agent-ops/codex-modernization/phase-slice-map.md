...
| --- | --- | --- | --- |
| 5 | v7 beta acceptance closure and public refresh | PXE-0105 done; PXE-0106 in progress; PXE-0074 partial | Beta.2 remains the last published/live candidate. The bounded beta.4 tracker/config correction slice now passes focused code/schema/dashboard gates: 8% point-click ROI authority, atomic detector retarget identity, thresholded recovery, effective CSRT settings, and explicit retirement of dead feature-matching options. Publish beta.4 only after broad release gates; then refresh the public VPS and repeat the operator retarget/recovery/UI checks. Continue fresh Ubuntu and physical Raspberry Pi Core then Full/model evidence under PXE-0074 without implying GStreamer source-build, production TLS, public WebRTC, QGC, PX4/SIH/SITL/HIL, field, follower-response, or aircraft readiness. |

...
| --- | --- | --- | --- |
| 5 | v7 beta acceptance closure and public refresh | PXE-0105, PXE-0074 | Automated gates and the bounded independent re-review are green for active/retry Classic retargeting, Smart repeated-click selection, tri-state Following controls, account/security event-loop isolation, config retirements, operator-state clarity, and media reconnect. Complete exact clean-checkout evidence, normal main merge, `v7.0.0-beta.1` prerelease, credential-preserving VPS refresh, and maintainer repeated-click acceptance without implying target-board or flight readiness. |

...
| --- | --- | --- | --- |
| Phase 5 guided update and Smart model recovery | complete | PXE-0153 | `checkpoints/2026-07-29-phase-5-update-smart-model-recovery.md`; repeated bootstrap explicitly reports preserved credentials and service/boot policy instead of replaying fresh onboarding. A typed missing-model Smart request now reveals an inline recovery control while Classic remains authoritative, supports empty inventory/cancel, and refreshes mode state before one model-selection retry. |
...
| Phase 5 authenticated guided browser and Windows Core preview | native authenticated CI complete; clean-host acceptance pending | PXE-0151 | `checkpoints/2026-07-29-phase-5-windows-core-preview-candidate.md`; guided Linux loopback/network and native Windows setup now use one `browser_session` profile, preserve existing credentials, and reserve raw `local_compat` for explicit loopback development. Exact candidate `c4b2c319` passed Windows run `30445705976`: owner-only credential ACL, unchanged credential hash on repeated setup, `64` native contracts, exact lifecycle, and decoded authenticated HTTP JPEG, WebSocket JPEG, and WebRTC before and after restart. A clean Windows 11 operator run and separate CPython 3.11 evidence remain. |
...
| Phase 5 saved tracker controls and gimbal packet coherence | implementation and local validation complete; real camera/gimbal retest pending | PXE-0145; PXE-0139 extension | `checkpoints/2026-07-25-phase-5-saved-tracker-controls-gimbal-coherence.md`; Dashboard Tracker and Follower controls now save through the canonical configuration path, apply without a process reboot, and distinguish a target-session handoff from a failed switch. Classic mode exposes the tracker selector; Smart mode exposes only the installed model selector. External gimbal angle and target-lock packets are composed from independently fresh components, side-effect-free status reads no longer mutate the update loop, and diagnostic output remains visible while follower readiness stays fail-closed. Local backend/dashboard/schema/build gates pass; real camera, gimbal, follower, PX4, Raspberry Pi, and field evidence remain separate. |
| Phase 5 guided update recovery and version policy | implementation and local validation complete; Ubuntu one-line acceptance pending | PXE-0144 | `checkpoints/2026-07-25-phase-5-guided-update-liveness-version-policy.md`; an interactive update now offers to stop an exactly owned PixEagle runtime before changing source or dependencies, while unattended updates require explicit opt-in and unknown ownership remains blocked. Generated dashboard settings backups are ignored operator data, with a narrow confirmed compatibility repair for affected checkouts. Source fetch and native builds publish cancellable low-noise heartbeats that retire after parent interruption. Transactional settings reset validates without a false missing-config warning. MAVSDK Server and MAVLink2REST remain reviewed manifest pins with SHA-256 verification; Python MAVSDK is pinned to the tested release. |
| Phase 5 verified component reuse and estimator/operator clarity | implementation and local validation complete; Ubuntu/Raspberry Pi/camera acceptance pending | PXE-0142; PXE-0143 deferred | `checkpoints/2026-07-25-phase-5-component-reuse-estimator-operator-clarity.md`; guided setup now reuses Python, AI, dlib, OpenCV/GStreamer, and dashboard components only after source-owned version and capability contracts pass, with one explicit advanced rebuild override. Config and dashboard environment reset are one validated backup/rollback transaction. Classic estimators seed at the selected target and use bounded monotonic time; prediction-only output stays visible but cannot drive followers. Follower confirmation is schema-backed and default-on. Classic Selection Assist is explicitly separate from SmartTracker's current `detect`/`obb` model boundary; Smart `segment` support is deferred without a false capability claim. Local setup, backend, full-suite, dashboard, schema, shell, and generated-contract gates pass. |
| Phase 5 degraded control-plane startup | implementation and local validation complete; Ubuntu unavailable-source acceptance pending | PXE-0141 | `checkpoints/2026-07-24-phase-5-degraded-control-plane-startup.md`; the API starts before one bounded external video-source activation, serialized reads cannot block source retirement, source failure remains visible through typed startup/media status, and reconnect runs outside the API event loop. Dashboard/backend panes are required while MAVLink2REST/MAVSDK sidecars may degrade in combined or separate tmux layouts without hiding Settings, Logs, or recovery. Core config/auth/API ownership and command-safety failures remain fail-closed. Local focused, setup/runtime, Phase 0, schema, static, and process-level unavailable-source gates pass; the operator's Ubuntu RTSP-failure rerun remains. |
| Phase 5 update/repair and runtime diagnostics | implementation and local validation complete; Ubuntu full-sidecar acceptance pending | PXE-0140 | `checkpoints/2026-07-23-phase-5-update-repair-runtime-diagnostics.md`; normal updates use the stopped-runtime `make update`/`pixeagle-service update` transaction, while a checkout already changed by raw `git pull` uses stopped-runtime `make repair`. Launcher preflight now classifies a missing MAVLink2REST binary and excludes that optional sidecar before tmux publication; later readiness failures preserve the exact structured-log handoff. Local lifecycle, setup/docs, API/reload, schema, shell, and no-MAVLink2REST backend/dashboard runtime gates pass; the operator's full Ubuntu sidecar rerun remains. |
| Phase 5 Gimbal live-switch lifecycle | implementation and local validation complete; physical provider acceptance pending | PXE-0139 | `checkpoints/2026-07-23-phase-5-gimbal-live-switch-lifecycle.md`; AppController now starts any external tracker's provider lifecycle before publishing a startup/live-switch/rollback result, cleans failed activation, and restores the previous tracker. Gimbal inactive state is structured instead of null, preventing per-frame warning floods. The persisted startup/restart tracker field is now a dropdown generated from the canonical tracker catalog and is explicitly separate from live runtime selection. No physical gimbal, follower, PX4, or field claim is made until real packets are observed. |
| Phase 5 tracker/follower robustness contract | implementation, exact-source CI, and VPS command-preview acceptance complete; maintainer browser and physical acceptance pending | PXE-0132 done; PXE-0131 and PXE-0133 deferred | `checkpoints/2026-07-22-phase-5-tracker-follower-robustness-contract.md`; Classic candidate validation no longer mutates confirmed geometry, estimator forecasts are observational, prediction stays command-ineligible, follower image-axis/PID signs and aim points are consistent, chase forward velocity uses a monotonic bounded ramp, gimbal chase exposes only its two implemented speed modes, dead settings and unsupported capability claims are removed, and active historical Canny thresholds are retained until value-preserving path migration exists. Full backend `3,518` passed / `48` optional skips / one tracked toolchain warning; dashboard `54` suites / `358` tests and build, schema `38/513`, focused contracts, static gates, deterministic real-OpenCV CSRT smoke, exact-source CI run `29929801579`, and deployed VPS preview evidence pass. Physical/aerial/PX4 evidence remains pending. |
...
| Phase 5 beta15 service/runtime, CI hygiene, and dependency-policy recovery | published; next acceptance gates open | PXE-0117, PXE-0118, PXE-0119 | `checkpoints/2026-07-21-phase-5-beta15-service-readiness-closure.md` and the service-runtime checkpoint; exact `ea725c76` closes the real Ubuntu update/systemd/tmux gate after fixing tracked-source chmod mutation, no-PX4 MAVSDK readiness, update-owned service actions, bounded restart recovery, and status clarity. Maintained GitHub Actions are now immutable-pinned with monthly Dependabot tracking, and the stale gimbal CI example is corrected. The Full AI/GStreamer host is clean and active with `NRestarts=0`, a healthy four-pane contract, owned loopback listeners, successful dashboard/About/MJPEG probes, and zero error-like active JSONL entries. Release-gate run `29809434623` passed and annotated prerelease `v7.0.0-beta.15` was published at `c34c8165`. Fresh beginner Ubuntu and exact-tag Raspberry Pi Core-first acceptance remain next; no PX4, QGC, remote WebRTC, field, or aircraft claim. |
| Phase 5 beta14 guided installer and GStreamer build acceptance recovery | done | PXE-0115; PXE-0116 deferred | `checkpoints/2026-07-21-phase-5-beta14-guided-installer-gstreamer-recovery.md`; beta.14 repaired guided input/defaults and strict source-build integrity, passed clean handoff, and was published with a bounded browser smoke. PXE-0118 later reconciled the real Ubuntu source-built provider and passed the maintained OpenCV GStreamer, QGC H.264/RTP/UDP element, effective `x264enc`, CSRT, and dlib capability checks. Receiver playback, Raspberry Pi performance, QGC receipt, PX4, and field gates remain separate. A reviewed model downloader remains deferred under PXE-0116. |
...
| --- | --- | --- | --- |
| 5 | v7.0.2 WebRTC lab recovery | PXE-0148/PXE-0149 done; PXE-0103 production follow-up | Auto now retains the ICE/media deadline until both a remote video track and connected ICE pair exist. The explicit browser lab records newly created TCP/UDP UFW rules in an owner-only receipt for exact rollback and cleanup. Local decoded-frame and release gates close the lab defect; production remote WebRTC still requires TLS/WSS, bounded TURN/firewall ownership, and target receiver evidence. |
| 5 | Accelerator runtime truth and target proof | PXE-0138 | Beta.26 replaces broad CUDA guessing with matrix-driven driver/compute-capability selection, executable kernel verification, truthful CPU fallback, and an operator compute badge. Next gate is strict-GPU readiness on the RTX 5080 host; Raspberry Pi CPU/NCNN and Jetson JetPack-specific acceleration require their own target evidence. |
| 5 | Time-aware aerial tracking acceptance | PXE-0131 | Preserve the current shared fail-closed measured/tentative/predicted/lost output contract while migrating Smart recovery from frame-count assumptions to monotonic elapsed-time evidence. Benchmark fixed-cadence and dropped-frame trajectories plus representative UAVDT/VisDrone small-target, ego-motion, scale, turn, occlusion, crossing, and distractor scenes before adding camera-motion, optical-flow, ReID, or alternate association backends. |
| 5 | Benchmark-gated aerial backend expansion | PXE-0123 | Do not implement a DETR/tiled/alternate-runtime backend from model-card enthusiasm alone. First benchmark supported YOLO detect/OBB baselines and candidates on retained representative aerial video and target hardware; proceed only for a material measured benefit, then integrate the complete artifact, lifecycle, normalized-output, tracking, config/API/UI, resource, and evidence contract. |

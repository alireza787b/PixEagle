...
| --- | --- | --- | --- |
| 5 | v7 beta acceptance closure and public refresh | PXE-0105 done; PXE-0074 partial | Automated gates, bounded independent re-review, candidate `cb3fec14` clean-checkout (`26/26`), normal main merge, exact versioned config retirement, and credential-preserving public refresh are complete. The beta.1 run exposed and beta.2 fixed a bounded Classic confidence roundoff defect. Exact beta.2 commit `98537984` is published and live; public API/security/MJPEG/WebSocket, Classic active retarget, two Smart override clicks, desktop/mobile overflow, browser error, cleanup, and log gates passed. Continue fresh Ubuntu and physical Raspberry Pi Core then Full/model evidence under PXE-0074 without implying GStreamer source-build, production TLS, public WebRTC, QGC, PX4/SIH/SITL/HIL, field, follower-response, or aircraft readiness. |
...
| 5 | Final release/handoff walkthrough | PXE-0074 | Partial pass complete: the maintained temporary-clone harness progressed from the 2026-07-07 `22/22` core lane and 2026-07-08 `25/25` dashboard lane to the exact candidate `cb3fec14` dashboard-inclusive `26/26` handoff. `v7.0.0-beta.2` at `98537984` is now tagged, published, and running on the public VPS with the bounded live browser/media/retarget/log gate passed. Next, record maintainer beta acceptance, run the public instructions from a fresh Ubuntu host, then capture physical Raspberry Pi Core and Full/model evidence. After the active public test session, remove temporary exposure and rotate/delete demo credentials; production and stable-release gates remain separate. |

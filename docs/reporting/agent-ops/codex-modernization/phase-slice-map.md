...
| 5 | Final release/handoff walkthrough | PXE-0074 | Partial pass complete: `tools/run_setup_handoff_walkthrough.py` now performs a repeatable clean temporary clone and 2026-07-07 evidence passed 22/22 dry-run/check-only commands for public docs/files, setup profile paths, quick-demo cleanup, binary plan, fast-forward-only sync, schema, and minimum backend/API tests. Before tag/release/handoff, rerun it on the exact release branch, add `--include-dashboard` for the final candidate or after frontend/tooling changes, complete public-demo cleanup/credential rotation after the active test session, capture production target evidence when selected, and fix any stale/noisy/confusing docs/scripts before tagging. |

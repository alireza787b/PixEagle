...
| Phase 4 typed tracking action foundation | in_progress | PXE-0064 partial | Current slice; typed `/api/v1/actions/tracking-start` and `/api/v1/actions/tracking-stop` action resources are being added with confirmation, dry-run, required idempotency for confirmed mutations, process-local action records, guarded non-callable API/MCP candidate classification, dashboard ROI start/stop migration, and local-only compatibility aliases for `/commands/start_tracking` and `/commands/stop_tracking` pending a later retirement gate |
...
hardening, typed replacements/retirement for the remaining legacy
tracking/control mutations, and broader adversarial browser/session/media
tests. No runtime MCP endpoint,
...
- PXE-0064 first containment foundation: backend/dashboard/MAVLink2REST
  ...
  legacy aliases, typed migration for still-legacy tracking/control mutations, and broader
  adversarial tests. Done in
  `checkpoints/2026-06-12-phase-4-api-exposure-containment.md`.
...
- PXE-0064 runtime auth foundation: route policy is enforced before HTTP/MJPEG
  ...
  retirement of remaining legacy aliases, typed migration for still-legacy
  tracking/control mutations, and broader adversarial tests.

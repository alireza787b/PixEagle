...
| Phase 4 runtime logging foundation | done/partial | PXE-0079 | `checkpoints/2026-07-04-phase-4-runtime-logging-foundation.md` and `checkpoints/2026-07-05-phase-4-runtime-logging-pane-capture.md`; PixEagle now creates per-run runtime manifests and `components/backend.jsonl`, redacts common credentials, applies bounded retention, exports a shared launcher run ID, captures `scripts/run.sh` launcher-piped output for started dashboard/sidecar components, exposes read-only typed `/api/v1/logs/status`, `/api/v1/logs/sessions`, and `/api/v1/logs/sessions/{run_id}`, and adds a dashboard Logs page for operator/debug visibility. Remaining PXE-0079 work is frontend error ingestion, live stream/export/evidence bundle, and clean setup evidence. Runtime logs are process-local evidence only, not PX4/SITL/HIL/field proof. |
...
| 4 | Unified runtime logging and evidence | PXE-0079 | Foundation plus launcher-piped component capture done: runtime sessions/manifests, backend JSONL, launcher-captured dashboard/sidecar component output, retention/redaction, launcher run ID, typed read-only `/api/v1/logs/*`, and dashboard Logs page. Remaining work: frontend error reports, live stream/export/evidence bundle, and final setup walkthrough evidence. Keep security audit separate and do not use runtime logs as flight proof without PX4/SITL/HIL artifacts. |

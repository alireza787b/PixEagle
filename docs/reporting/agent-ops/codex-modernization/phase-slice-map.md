...
until head `b98848b2c` has resolved or documented QGC CI results and target
receiver evidence is accepted. After that, resume the planned Phase 4 cleanup order:
...
| 4 | QGC authenticated remote HTTP/WS media | PXE-0070 | Keep PR #13594 draft until clean CI and target receiver evidence exist; monitor head `b98848b2c` after the appsrc-pad WebSocket JPEG delivery repair and VideoSettings fixture-lifetime fix, resolve or document any remaining CI failures, then validate generic anonymous sources and authenticated PixEagle HTTPS/WSS media before advertising remote compatibility. |
...
| 5 | Final release/handoff walkthrough | PXE-0074 | Before any tag/release/handoff, clone or copy to a clean temporary directory and follow the public README, bootstrap, setup, update, and configuration docs exactly for beginner demo and senior-dev override paths; fix stale, noisy, confusing, or deprecated docs/scripts before tagging. |

...
| Phase 4 setup-profile foundation | in_progress | PXE-0068 partial | `checkpoints/2026-06-17-phase-4-setup-profile-foundation.md`; added explicit setup-profile tooling, `local_dev` and `field_qgc_video` profile application, fail-closed defined remote/unsafe profiles, Make targets, QGC UDP/RTP default port `5600` alignment, init/install clean-clone default behavior, deployment-only service opt-in, Windows port fallback, companion/troubleshooting cleanup, legacy telemetry WebSocket labeling, and setup-profile/docs regression guards; PXE-0068 remains open for binary download pin/override/checksum/provenance policy and future credential-generating remote profile automation |
...
| 4 | Bootstrap/setup UX cleanup | PXE-0068 | Reconcile install/init/service/binary/download docs and scripts into explicit demo/dev, advanced dev, and deployment/service profiles; remove stale script refs, config drift, port-label confusion, and sidecar/SITL contradictions; add docs guardrails. |

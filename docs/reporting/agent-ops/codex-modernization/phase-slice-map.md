...
work now focuses on typed tracker configuration mutation design, typed
replacement design for the remaining schema/capabilities diagnostic reads,
and any future route-boundary debt discovered by static guards. The public
...
`GET /api/v1/tracking/telemetry`; only legacy `GET /api/tracker/schema` and
`GET /api/tracker/capabilities` remain registered under the tracker diagnostic

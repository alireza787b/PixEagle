...
| 4 | Controlled user VPS/browser handoff | PXE-0068, PXE-0074 | Next recommended slice: use the completed clean-walkthrough evidence to start the reviewed branch with SSH/local-only or private-overlay/lab access, verify binds and typed reads after the selected posture/profile is applied, hand off the browser URL and any credentials outside the repository, and record sanitized evidence. Public HTTPS/WSS production access remains a separate TLS/proxy/firewall evidence path. |

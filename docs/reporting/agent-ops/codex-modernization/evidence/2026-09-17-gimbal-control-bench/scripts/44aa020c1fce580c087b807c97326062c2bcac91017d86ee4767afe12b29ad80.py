...
def main():
    ...
    parser.add_argument('phase', choices=['identity', 'queries', 'routing', 'video', 'observe', 'describe',
                        'qt-point', 'rotated-point', 'ai-observe', 'ai-select', 'stops', 'tracking-ready', 'pan', 'tilt', 'pan-step', 'tilt-step', 'zoom', 'home', 'point', 'box'])
    ...
    parser.add_argument('--descriptor', type=int, choices=[0,1,8,9], default=None)
    parser.add_argument('--track-seconds', type=int, choices=[1,3,4,6], default=1)
    parser.add_argument('--size-pixels', type=int, choices=[64,128], default=64)
    args = parser.parse_args()
    if not (0.05 <= args.x <= .95 and .05 <= args.y <= .95): parser.error('Selection outside bench limits')
    control = args.phase in ('qt-point', 'rotated-point', 'ai-observe', 'ai-select', 'stops', 'tracking-ready', 'pan', 'tilt', 'pan-step', 'tilt-step', 'zoom', 'home', 'point', 'box')
    ...
    try:
        if args.phase in ('identity', 'queries', 'routing') or control:
            ...
            def poll(seconds):
                ...
                while time.monotonic() < end:
                    ...
                    receive(0.10 if args.phase == 'qt-point' else 0.12)
                    send(QUERIES[1], 'query_body_angles')
                    receive(0.10 if args.phase == 'qt-point' else 0.28)
            ...
            if control:
                ...
                if args.phase == 'rotated-point':
                    cleanup.append(b'#TPPD2wROT00')
                ...
                def independent_stop():
                    ...
                    if stop_event.wait(18 if args.phase in ('ai-observe', 'ai-select') else 15):
                        return
                ...
                poll(0.8)
                if args.phase == 'rotated-point':
                    send(b'#TPPD2wROT02', 'temporary_camera_rotation_180')
                    send(b'#TPPD2rROT00', 'query_camera_rotation')
                    poll(1.2)
                if args.phase in ('ai-observe', 'ai-select'):
                    ...
                    poll(0.8)
                elif args.phase in ('tracking-ready', 'point', 'box', 'rotated-point', 'qt-point'):
                    send(b'#TPPD2wTRC02', 'prepare')
                    poll(0.8)
                    if last_status[0] != '01':
                        raise RuntimeError(f'Prepare did not produce ready state: {last_status[0]}')
                    if args.phase in ('point', 'box', 'rotated-point', 'qt-point'):
                        width, height, flag = (round(args.size_pixels/1920*2000), round(args.size_pixels/1080*2000), 9) if args.phase in ('point', 'rotated-point') else (200, 200, 1)
                        if args.phase == 'qt-point':
                            width, height, flag = 62, 111, 9
                        if args.descriptor is not None:
                            flag = args.descriptor
                        cx, cy = round((args.x - 0.5)*2000), round((args.y - 0.5)*2000)
                        if args.phase == 'qt-point':
                            cx, cy = int((int(args.x*1920)-960)/.96), int((int(args.y*1080)-540)/.54)
                    ...
                    poll(0.8)
                if args.phase == 'rotated-point':
                    send(b'#TPPD2wROT00', 'restore_camera_rotation_0')
                    send(b'#TPPD2rROT00', 'verify_camera_rotation_restored')
                    receive(0.4)
            ...
            event('passive_begin')
            receive(2)
            bodies = ([b'#TPPD2rROT00', b'#tpPG2rGMS00', b'#TPPG2rVER00', b'#TPPD2rVSN00'] if args.phase == 'identity' else QUERIES)
            steps = [(sock, body) for sock in sockets for body in bodies]

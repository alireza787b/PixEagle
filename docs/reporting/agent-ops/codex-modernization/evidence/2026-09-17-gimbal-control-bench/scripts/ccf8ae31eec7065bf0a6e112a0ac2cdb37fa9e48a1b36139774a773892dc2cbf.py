...
def main():
    ...
    parser.add_argument('phase', choices=['queries', 'routing', 'video', 'observe', 'describe',
                        'stops', 'tracking-ready', 'pan', 'tilt', 'zoom', 'home', 'point', 'box'])
    parser.add_argument('--execute-standalone-bench', action='store_true')
    parser.add_argument('--pulse-ms', type=int, choices=[150, 250], default=150)
    parser.add_argument('--speed', type=int, choices=[5, 10], default=5)
    args = parser.parse_args()
    control = args.phase in ('stops', 'tracking-ready', 'pan', 'tilt', 'zoom', 'home', 'point', 'box')
    ...
    try:
        if args.phase in ('queries', 'routing') or control:
            ...
            if control:
                ...
                poll(0.8)
                if args.phase in ('tracking-ready', 'point', 'box'):
                    ...
                    if args.phase in ('point', 'box'):
                        ...
                        payload = b'#tpPDAwLOC' + struct.pack('>hhhhBB', 0, 0, width, height, 0, flag)
                        send(payload, 'select_center_' + args.phase)
                        poll(1.2)
                    send(b'#TPPD2wTRC01', 'cancel_to_ready')
                    poll(0.8)
                    send(disable, 'disable')
                    poll(0.8)
                elif args.phase in ('pan', 'tilt', 'zoom'):
                    commands = {
                    ...
                    positive, negative, halt = commands[args.phase]

...
def main():
    ...
    parser.add_argument('phase', choices=['queries', 'routing', 'video', 'observe', 'describe',
                        'stops', 'tracking-ready', 'pan', 'tilt', 'zoom', 'point', 'box'])
    parser.add_argument('--execute-standalone-bench', action='store_true')
    args = parser.parse_args()
    control = args.phase in ('stops', 'tracking-ready', 'pan', 'tilt', 'zoom', 'point', 'box')
        ...
        if args.phase in ('queries', 'routing') or control:
            ...
            if control:
                ...
                if args.phase in ('tracking-ready', 'point', 'box'):
                    ...
                    commands = {
                        'pan': (b'#TPUG2wGSY05', b'#TPUG2wGSYFB', movement_stop),
                        'tilt': (b'#TPUG2wGSP05', b'#TPUG2wGSPFB', movement_stop),
                        'zoom': (b'#TPPM2wZMC02', b'#TPPM2wZMC01', zoom_stop),
                    }
                    positive, negative, halt = commands[args.phase]
                    for body in (positive, negative):
                        send(body, args.phase + '_pulse_150ms')
                        receive(0.15)

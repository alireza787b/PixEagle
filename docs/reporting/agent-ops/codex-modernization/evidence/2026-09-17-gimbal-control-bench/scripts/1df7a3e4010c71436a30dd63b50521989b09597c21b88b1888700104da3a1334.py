...
def main():
    ...
    parser.add_argument('phase', choices=['identity', 'queries', 'routing', 'video', 'observe', 'describe',
                        'rotated-point', 'ai-observe', 'ai-select', 'stops', 'tracking-ready', 'pan', 'tilt', 'pan-step', 'tilt-step', 'zoom', 'home', 'point', 'box'])
    ...
    control = args.phase in ('rotated-point', 'ai-observe', 'ai-select', 'stops', 'tracking-ready', 'pan', 'tilt', 'pan-step', 'tilt-step', 'zoom', 'home', 'point', 'box')
        ...
        if args.phase in ('identity', 'queries', 'routing') or control:
            ...
            if control:
                ...
                if args.phase in ('ai-observe', 'ai-select'):
                    ...
                    poll(0.8)
                elif args.phase in ('tracking-ready', 'point', 'box', 'rotated-point'):
                    send(b'#TPPD2wTRC02', 'prepare')
                    poll(0.8)
                    if last_status[0] != '01':
                        raise RuntimeError(f'Prepare did not produce ready state: {last_status[0]}')
                    if args.phase in ('point', 'box', 'rotated-point'):

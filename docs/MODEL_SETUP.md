...
The system-level unit installed by `sudo pixeagle-service install` is the
maintained integration for this requirement. It sets systemd `Delegate=yes`,
which permits the non-root PixEagle service to manage only its own cgroup
subtree; it does not change `User=`, add Linux capabilities, or grant general
host privilege. Boot enablement is independent. Manual tmux runs and other
non-systemd service managers are not automatically delegated and are not
claimed as NCNN-export-capable. They fail closed unless the external launcher
independently supplies an owner-delegated cgroup-v2 boundary.

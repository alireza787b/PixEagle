...
REM ============================================================================
REM download_mavsdk_server.bat - MAVSDK Server Downloader for Windows
REM ============================================================================
REM Downloads the MAVSDK Server binary for Windows.
REM
REM Usage: download_mavsdk_server.bat
REM ============================================================================
...
REM Configuration
set "MAVSDK_VERSION=v3.14.0"
set "GITHUB_BASE_URL=https://github.com/mavlink/MAVSDK/releases/download"
set "BINARY_NAME=mavsdk_server_win_x64.exe"
set "BINARY_URL=%GITHUB_BASE_URL%/%MAVSDK_VERSION%/%BINARY_NAME%"
set "BINARY_PATH=%PIXEAGLE_DIR%\mavsdk_server_bin.exe"

REM Display header
echo.
echo ============================================================
echo   MAVSDK Server Downloader
echo ============================================================
echo   Version: %MAVSDK_VERSION%
...
REM Check architecture and select appropriate binary
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
    call :print_ok "Architecture: x64 (AMD64)"
    set "BINARY_NAME=mavsdk_server_win_x64.exe"
) else if "%PROCESSOR_ARCHITECTURE%"=="ARM64" (
    call :print_ok "Architecture: ARM64"
    set "BINARY_NAME=mavsdk_server_win_arm64.exe"
) else if "%PROCESSOR_ARCHITECTURE%"=="x86" (
    if "%PROCESSOR_ARCHITEW6432%"=="AMD64" (
        call :print_ok "Architecture: x64 (via WoW64)"
        set "BINARY_NAME=mavsdk_server_win_x64.exe"
    ) else (
        call :print_ok "Architecture: x86"
        set "BINARY_NAME=mavsdk_server_win_x86.exe"
    )
) else (
    call :print_warn "Unknown architecture: %PROCESSOR_ARCHITECTURE%"
    call :print_info "Defaulting to x64 binary"
    set "BINARY_NAME=mavsdk_server_win_x64.exe"
)

REM Update URL with correct binary
set "BINARY_URL=%GITHUB_BASE_URL%/%MAVSDK_VERSION%/%BINARY_NAME%"
...
if exist "%BINARY_PATH%" (
    ...
    echo.
    set /p "REPLY=       Replace with %MAVSDK_VERSION%? [y/N]: "
    echo.
...
call :print_info "Version: %MAVSDK_VERSION%"
call :print_info "Size: ~15 MB"
echo       URL: %BINARY_URL%
echo.
...
REM Check file size (should be > 1MB for valid executable)
...
REM Skip execution test - MAVSDK server is a daemon that doesn't exit on --version
...
echo ============================================================
echo.
echo   %CHECK% MAVSDK Server %MAVSDK_VERSION%
echo   %CHECK% Platform: Windows %PROCESSOR_ARCHITECTURE%
...
echo ============================================================
echo.
echo   1. Visit MAVSDK Releases:
echo      %CYAN%https://github.com/mavlink/MAVSDK/releases/tag/%MAVSDK_VERSION%%NC%
echo.
...
echo      - mavsdk_server_win_x64.exe   (64-bit)
echo      - mavsdk_server_win_x86.exe   (32-bit)
echo      - mavsdk_server_win_arm64.exe (ARM64)
echo.
echo ============================================================
echo.
pause

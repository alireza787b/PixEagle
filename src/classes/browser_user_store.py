...
def _read_user_file_bytes(path: Path) -> bytes:
    return read_owner_only_bytes(
        path,
        label="API session user file",
        error_type=BrowserUserPersistenceError,
    )


def _validate_open_file_status(file_status, path: Path) -> None:
    validate_open_file_status(
        file_status,
        path,
        label="API session user file",

...
class GMVelocityVectorFollower(BaseFollower):
    ...
    def follow_target(self, tracker_output: TrackerOutput) -> bool:
        ...
        try:
            ...
            if tracker_output.tracking_active:
                ...
                if self._should_stop_for_unusable_external_input(tracker_output):
                    return self._stop_for_unusable_external_input(tracker_output)
    ...
    def _should_stop_for_unusable_external_input(self, tracker_output: TrackerOutput) -> bool:
        """Return True when an external gimbal output must fail closed immediately."""
        raw_data = tracker_output.raw_data or {}
        metadata = tracker_output.metadata or {}

        return bool(
            raw_data.get('data_is_stale') or
            metadata.get('stale_data') or
            raw_data.get('usable_for_following') is False or
            metadata.get('usable_for_following') is False
        )

    def should_process_inactive_tracker_output(self, tracker_output: TrackerOutput) -> bool:
        """
        Allow AppController to route unusable external gimbal output here.

        This keeps the real command path able to dispatch the zero velocity
        command generated by `_stop_for_unusable_external_input()`.
        """
        return (
            self._should_stop_for_unusable_external_input(tracker_output) or
            self._is_inactive_tracker_output(
                tracker_output,
                allowed_types={TrackerDataType.GIMBAL_ANGLES},
            )
        )

    def _stop_for_unusable_external_input(self, tracker_output: TrackerOutput) -> bool:
        """Zero body-velocity commands for stale or unusable external gimbal input."""
        raw_data = tracker_output.raw_data or {}
        reason = raw_data.get('inactive_reason') or raw_data.get('tracking_status') or 'unusable_external_gimbal_input'
        ...
        self.following_active = False
        self.failed_updates = getattr(self, 'failed_updates', 0) + 1
        self.log_follower_event("external_gimbal_input_fail_closed", reason=reason)
        logger.warning("External gimbal input unusable; zero velocity command emitted (%s)", reason)
        return True
    ...
    def _handle_target_loss(self, current_time: float) -> bool:
        ...
        if time_since_loss < self.target_loss_timeout:
            ...
            if self.enable_velocity_decay and self.last_velocity_vector is not None:
                ...
                if not self.set_command_fields(
                    {
                        "vel_body_fwd": decayed_vector.x,
                        "vel_body_right": decayed_vector.y,
                        "vel_body_down": decayed_vector.z,
                        "yawspeed_deg_s": 0.0,
                    },
                    reason='gm_velocity_vector_target_loss_decay',
                ):
                    return False
            ...
            if not self.set_command_fields(
                ...
                reason='gm_velocity_vector_target_loss_stop',
            ):
                return False
            self.current_velocity_magnitude = 0.0
            self.last_velocity_vector = None
